...
#define USAGE "USAGE:\n"\
              "  dzr [TRACK...]\n"\
              "ENVIRONMENT:\n"\
              "  - FMT: MP3_128 = 0, MP3_320 = 3, AAC_96 = 8, FLAC=9\n"\
              "  - sid=%s: your session id cookie\n"\
...
int fetch(char *meth, char *path, char *params, char *host, char **fields, char **bodies, char *out, size_t outmax,
          int(*cb)(SSL*, ssize_t, void**), void**cb_args) {
	int clen = 0, len = 0, tcp = socket(AF_INET, SOCK_STREAM, 0);
	...
	for (int i = 0; header && header[i]; i++) http_send(ssl, header[i]);
...
int decryptTrack(SSL* ssl, ssize_t mp3_size, void**args) {
	...
	while ((ret = http_recv(ssl, chunk + (total % sizeof(chunk)), sizeof(chunk) - (total % sizeof(chunk)))) > 0) {
		...
		if (fwrite(chunk, sizeof(chunk), 1, fd) < 0)break;
...
int main(int argc, char *argv[]) {
	...
	char *dbg = getenv("DEBUG"), *bf=getenv("DZR_CBC"), *aes=getenv("DZR_AES"), *fmt = getenv("FMT") ?: "0";
	if (argc < 1 || !sid || !bf || !aes)
		return fprintf(stderr, USAGE, sid, bf, aes);
	...
	if (debug)fprintf(stderr, "sid=%s FMT=%s token=%s\n", sid, fmt, token);
